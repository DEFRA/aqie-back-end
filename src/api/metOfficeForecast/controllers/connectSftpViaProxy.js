...
export async function connectSftpThroughProxy() {
  return new Promise((resolve, reject) => {
    try {
      ...
      const proxyPort = parseInt(new URL(config.get('httpsProxy')).port || '443');
      const sftpHost = 'sftp22.sftp-server-gov-uk.quatrix.it';
      const sftpPort = 22;

      console.log(`[Proxy Debug] Using proxy ${proxyHost}:${proxyPort}`);
      console.log(`[Proxy Debug] Attempting to create tunnel to ${sftpHost}:${sftpPort}`);
      ...
      const timeout = setTimeout(() => {
        console.error('[Tunnel Timeout] SFTP proxy connection timed out after 10s');
        reject(new Error('Tunnel connection timed out'));
      }, 10000); // 10 seconds

      tunneler.createSocket({ host: sftpHost, port: sftpPort }, (err, socket) => {
